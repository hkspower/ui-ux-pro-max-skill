...
`Content/Models/Ahmed.fbx` (and `Ahmed.gltf` + `Ahmed.bin`) is a rigged,
textured Ahmed,
...
```bash
pip install bpy
python3 Tools/blender/build_ahmed.py            # the hero: 4K colour, 2K normals, 64-sample renders (~15 min)
...
`../Textures/Ahmed/*.png` by relative path, and the glTF export is checked
...
relative paths to `Content/Textures/Ahmed` rather than embedded copies, so
...
is not one of the nine districts: `L_Prologue`, Ahmed's life before he fell,
...
own `AAhmedPrologueGameMode`, set on that level's World Settings rather than
