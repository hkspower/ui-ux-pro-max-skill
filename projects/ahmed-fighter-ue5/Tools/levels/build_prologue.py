"""AHMED — Kuwait Fighter :: build L_Prologue
==============================================================================
The one level that is not one of the nine. Ahmed's life before he fell: a
title bout, then a short walk to the hole in the street. It is never on the
ring map and it is never revisited -- AAhmedPrologueGameMode sends a player
...
more row it plans. Three reasons:
  ...
  * build_levels.py's own blockout is still written to the strip this game
    ...
    since AhmedArena.h. This script is written to the model that replaced
    it -- a circular arena wherever the wave wakes, AhmedGameplay::ArenaRadius
...
GAME_MODE_PATH = "/Script/AhmedFighter.AhmedPrologueGameMode"

# Ahmed's own gym, not one of the nine districts, so it takes its own rig
...
CAGE_RADIUS = 1150.0   # just outside AhmedGameplay::ArenaRadius (1100 cm)
STAND_Y = 1500.0
...
def plan():
    ...
    # circle, dressing only -- AAhmedCharacter::SetArenaCircle is what
...
def describe(actors):
    print("\n%s  (Ahmed's life before he fell, %d cm, never on the ring map)" %
...
def build(actors):
    ...
    # This level's GameMode is AAhmedPrologueGameMode, not the project
