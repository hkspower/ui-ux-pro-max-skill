...
WHY THIS EXISTS. The Unreal build has no animation at all. AhmedTypes.h says
...
already knows:
  ...
  SHAPE     the browser build's own attackPose(), index.html:1347. It draws
            ...
            from.
  WHO THEY  ../../ahmed-fighter/CLAUDE.md -- the canon. ZAYOS "only ever
...
is AhmedAttackAbility.cpp:85, on the ability path -- and nothing in C++ ever
...
per-ATTACK pointer (AhmedTypes.h:147) and a boss's jab is not Ahmed's jab, so
...
#
# `lead` is the left side and `rear` the right: GUARD in build_ahmed.py puts
...
# ====================================================== the strike poses
# Written the way build_ahmed.GUARD and KICK are: a dict of world-space aims,
...
# own curve. Ahmed faces -Y, +X is his left, Z is up.
#
...
ENEMIES = os.path.abspath(os.path.join(PROJECT, "..", "ahmed-fighter", "assets", "enemies.js"))
...
def browser_build(kind):
    ...
    bosses ride Ahmed's one rig at Ahmed's proportions, so ZAYOS being "half
...
BROWSER = os.path.abspath(os.path.join(PROJECT, "..", "ahmed-fighter", "index.html"))
...
def reaim(rig, aims, names):
    ...
    thing build_ahmed.pose() does, applied again after the torso has been
...
def build(clips, out_dir, sheet=False):
    ...
    # a skeleton it already has, and the mesh is Ahmed's and already there.
    for c, act in made:
        rig.animation_data.action = act
