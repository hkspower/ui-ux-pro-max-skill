...
Run through build_ahmed.py for the hero, build_fighters.py for anyone:
...
ONE BODY, EVERY MAN. Everything up to the bake runs at Ahmed's coordinates
...
# on the one it calls the smallest man on screen. Ahmed is the skull as
# sculpted.
FACES = {
    "brawler": dict(brow=1.35, glabella=1.20, jaw1=1.30, jaw2=1.30, jaw3=1.30, chin=1.25,
                    masseter=1.50, cheekbone=1.15, tip=1.10, hollow=0.60),
    "thug":    dict(chin=0.85, brow=0.90, cheekbone=0.90, tip=0.95, masseter=0.80),
}


def run(argv=None):
    """The hero, as build_ahmed.py has always built him."""
    from . import roster
    return build_fighter(roster.spec("ahmed"), argv)
...
def build_fighter(spec, argv=None):
    ...
    CHECK = os.path.join(OUT, "built.blend" if low == "ahmed" else "built_%s.blend" % low)
    ...
    # ---- his own size. Everything above ran at Ahmed's coordinates; from
    ...
    if low == "ahmed":
        ...
        # skeleton build_armature makes from the joint table as it is. Ahmed
        ...
        assert {b.name for b in rig.data.bones} == {b.name for b in ref.data.bones} and worst < 1e-6, \
            "Ahmed's skeleton drifted %.2e m from the one the boss clips are keyed to" % worst
    ...
    # Ahmed_Rig; every man exports under it, whatever his .blend calls him.
    rig.name = "Ahmed_Rig"; rig.data.name = "AhmedSkeleton"
