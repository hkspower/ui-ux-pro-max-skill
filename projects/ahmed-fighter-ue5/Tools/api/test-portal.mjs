...
const tmp = mkdtempSync(join(tmpdir(), 'ahmed-portal-'));
...
const server = spawn(process.execPath, [join(HERE, 'server.mjs')], {
  env: { ...process.env, PORT: String(PORT), HOST: '127.0.0.1',
         AHMED_CONFIG: CONFIG, AHMED_SAVES: SAVES, SAVE_TOKEN: 'sesame',
...
function keep(r){
  ...
  for(const c of set){
    ...
    if(k.trim() === 'ahmed_portal') cookie = v === '' ? '' : `ahmed_portal=${v}`;
  }
  return r;
}
...
try {
  ...
  const spawnWith = async (env, port) => {
    const p = spawn(process.execPath, [join(HERE, 'server.mjs')], {
      env: { ...process.env, PORT: String(port), HOST: '127.0.0.1',
             AHMED_CONFIG: CONFIG, AHMED_SAVES: SAVES, ...env },
      stdio: ['ignore', 'pipe', 'inherit']
    });
