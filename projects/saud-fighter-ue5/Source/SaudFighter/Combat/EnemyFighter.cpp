...
void AEnemyFighter::EnterPhaseTwo()
{
	...
	if (USaudAudioSubsystem* Audio = USaudAudioSubsystem::Get(this)) { Audio->Play(TEXT("Boss_Enrage"), this); }
	BP_OnEnraged();
}

void AEnemyFighter::TickAI(float DeltaSeconds, ASaudCharacter* Player)
{
	...
	FVector Desired = SaudArena::FlankSpot(Target, Self, CrowdBearing, Range, LaneOffset);

	// Far away, forget the bearing and just close.
	if (SaudArena::Flat(Delta) > 950.f)
	{
		Desired = Target;
	}
	...
	for (TActorIterator<AEnemyFighter> It(GetWorld()); It; ++It)
	{
		...
		Desired = SaudArena::PushApart(Desired, Other->GetActorLocation(), 140.f);
	}
	Desired = SaudArena::ClampToCircle(FVector(Desired.X, Desired.Y, Self.Z),
