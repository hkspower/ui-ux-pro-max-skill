...
void AFighterBase::ReceiveKnockback(const FVector& Impulse, bool bKnockdown)
{
	...
	if (bKnockdown && !bResistsKnockdown)
	{
		...
		if (AbilitySystem)
		{
			...
			AbilitySystem->AddLooseGameplayTag(SaudTags::State_Downed);
			AbilitySystem->SendCombatEvent(SaudTags::Event_Knockdown, nullptr, 0.f);
		}
		OnKnockedDown();
	}
	else
	{
		...
		if (AbilitySystem)
		{
			AbilitySystem->AddLooseGameplayTag(SaudTags::State_HitStun);
		}
	}
}
...
void AFighterBase::Tick(float DeltaSeconds)
{
	...
	if (AbilitySystem)
	{
		if (HitStunRemaining <= 0.f) { AbilitySystem->RemoveLooseGameplayTag(SaudTags::State_HitStun); }
		if (DownRemaining <= 0.f)    { AbilitySystem->RemoveLooseGameplayTag(SaudTags::State_Downed); }
	}
...
bool AFighterBase::StartAttack(FName AttackRow)
{
	...
	if (!Attack)
	{
		UE_LOG(LogTemp, Warning, TEXT("[Saud] Unknown attack row '%s'"), *AttackRow.ToString());
		return false;
	}
...
FHitResultData AFighterBase::ReceiveHit(AFighterBase* Attacker, const FAttackDef& Attack)
{
	...
	const bool bFacingAttacker = SaudArena::Covers(Facing, ToAttacker);
	...
	if (bGuarding)
	{
		Damage *= SaudGameplay::BlockDamageMultiplier;
	}
...
void AFighterBase::PlayFootstep()
{
	...
	if (USaudAudioSubsystem* Audio = USaudAudioSubsystem::Get(this)) { Audio->Play(TEXT("Footstep"), this); }
}
...
void AFighterBase::FaceTowards(const FVector& WorldLocation)
{
	...
	const FVector Wanted = SaudArena::Direction(GetActorLocation(), WorldLocation, Facing);
...
void AFighterBase::OnDeath()
{
	...
	if (USaudAudioSubsystem* Audio = USaudAudioSubsystem::Get(this)) { Audio->Play(TEXT("KO"), this); }
