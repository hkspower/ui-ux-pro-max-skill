...
#include "SaudTypes.generated.h"

class USaudFightStyleData;
class UAnimMontage;
class UStaticMesh;
class UWorld;
...
{
	SAUDFIGHTER_API FGameplayTag TagFor(EAbility Ability);
	SAUDFIGHTER_API EAbility AbilityFor(const FGameplayTag& Tag);
}
...
struct FFighterDef : public FTableRowBase
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "AI")
	TSoftObjectPtr<USaudFightStyleData> FightStyle;
...
namespace SaudGameplay
{
	...
	/** How far in from a district's rim a door or gate stands, so neither
	    sits exactly on the edge SaudArena::DistrictExtent draws. Shared
