...
#include "SaudSaveSyncSubsystem.generated.h"
 ...
 *
 * The game's save is USaudGameInstance's FSaudProgress in a local slot,
 ...
 *
 * The body is FSaudProgress as JSON under the same two keys the server
 ...
 * [/Script/SaudFighter.SaudSaveSyncSubsystem].
 */
UCLASS(Config = Game)
class SAUDFIGHTER_API USaudSaveSyncSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	static bool EncodeProgress(const FSaudProgress& In, FString& OutJson);
	static bool DecodeProgress(const FString& Json, FSaudProgress& Out, FString& OutError);

protected:
	/** Base address of the API, no trailing slash. Empty disables sync.
	    Usually the same server as USaudConfigSubsystem::BalanceApiUrl. */
