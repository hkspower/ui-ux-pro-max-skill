...
DEFINE_LOG_CATEGORY_STATIC(LogSaudBalance, Log, All);

void USaudConfigSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
...
void USaudConfigSubsystem::Deinitialize()
{
	bInFlight = false;
	Super::Deinitialize();
}
...
void USaudConfigSubsystem::UseBakedTables()
{
...
FString USaudConfigSubsystem::CachePath() const
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Balance"), TEXT("config.json"));
}

void USaudConfigSubsystem::TryCachedPayload()
{
	...
	{
		UE_LOG(LogSaudBalance, Warning, TEXT("Cached balance unreadable at %s."), *Path);
		return;
	}
	...
	{
		UE_LOG(LogSaudBalance, Log, TEXT("Applied cached balance, revision %s."), *Revision);
		OnBalanceApplied.Broadcast(Source);
	}
	else
	{
...
void USaudConfigSubsystem::WriteCache(const FString& Json) const
{
	const FString Path = CachePath();
	if (!FFileHelper::SaveStringToFile(Json, *Path))
	{
		UE_LOG(LogSaudBalance, Warning, TEXT("Could not write balance cache to %s."), *Path);
	}
}
...
USaudConfigSubsystem::MakeRequest(const FString& Path) const
{
	...
	Req->SetHeader(TEXT("User-Agent"),
		FString::Printf(TEXT("SaudFighter/%s"), *FApp::GetBuildVersion()));
	Req->SetTimeout(RequestTimeoutSeconds);
	return Req;
}

void USaudConfigSubsystem::RefreshFromServer()
{
	if (!bEnableRemoteBalance || BalanceApiUrl.IsEmpty())
	{
		UE_LOG(LogSaudBalance, Log, TEXT("Refresh ignored: remote balance is off."));
		return;
	}
	...
	if (bInFlight)
	{
		UE_LOG(LogSaudBalance, Verbose, TEXT("Refresh ignored: a fetch is already in flight."));
		return;
	}
	RequestRevision();
}

void USaudConfigSubsystem::RequestRevision()
{
	bInFlight = true;
	TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> Req = MakeRequest(TEXT("/v1/revision"));
	Req->OnProcessRequestComplete().BindUObject(this, &USaudConfigSubsystem::OnRevisionResponse);
	Req->ProcessRequest();
}

void USaudConfigSubsystem::OnRevisionResponse(FHttpRequestPtr, FHttpResponsePtr Res, bool bOk)
{
	...
	{
		bInFlight = false;
		UE_LOG(LogSaudBalance, Warning, TEXT("Revision response was not JSON."));
		return;
	}
	...
	{
		bInFlight = false;
		UE_LOG(LogSaudBalance, Warning, TEXT("Revision response carried no Revision."));
		return;
	}
	...
	{
		bInFlight = false;
		UE_LOG(LogSaudBalance, Log, TEXT("Balance already current at revision %s."), *Revision);
		return;
	}

	UE_LOG(LogSaudBalance, Log, TEXT("Balance %s -> %s; fetching."),
		Revision.IsEmpty() ? TEXT("(none)") : *Revision, *Remote);
	RequestConfig();
}

void USaudConfigSubsystem::RequestConfig()
{
	...
	Req->OnProcessRequestComplete().BindUObject(this, &USaudConfigSubsystem::OnConfigResponse);
	Req->ProcessRequest();
}

void USaudConfigSubsystem::OnConfigResponse(FHttpRequestPtr, FHttpResponsePtr Res, bool bOk)
{
	bInFlight = false;

	if (!bOk || !Res.IsValid())
	{
		UE_LOG(LogSaudBalance, Log, TEXT("Balance fetch failed; keeping what we have."));
		return;
	}
	if (Res->GetResponseCode() == 304)
	{
		UE_LOG(LogSaudBalance, Log, TEXT("Balance unchanged (304)."));
		return;
	}
	if (Res->GetResponseCode() != 200)
	{
		UE_LOG(LogSaudBalance, Warning, TEXT("Balance fetch returned %d."), Res->GetResponseCode());
		return;
	}
	...
	if (!ApplyPayload(Json, EBalanceSource::Remote, Error))
	{
		...
		UE_LOG(LogSaudBalance, Warning, TEXT("Balance payload rejected (%s)."), *Error);
		return;
	}

	WriteCache(Json);
	UE_LOG(LogSaudBalance, Log, TEXT("Balance updated to revision %s."), *Revision);
	OnBalanceApplied.Broadcast(Source);
}
 ...
 */
bool USaudConfigSubsystem::ApplyPayload(const FString& Json, EBalanceSource NewSource, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
