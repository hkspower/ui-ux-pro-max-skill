#include "Gameplay/SaudGameplayEffects.h"
...
UGE_SaudDamage::UGE_SaudDamage()
{
	...
	Modifiers.Add(MakeSetByCaller(USaudAttributeSet::GetIncomingDamageAttribute(),
		SaudTags::Data_Damage));
}

UGE_SaudAttackCost::UGE_SaudAttackCost()
{
	DurationPolicy = EGameplayEffectDurationType::Instant;
	...
	Modifiers.Add(MakeSetByCaller(USaudAttributeSet::GetStaminaAttribute(),
		SaudTags::Data_StaminaCost));
	Modifiers.Add(MakeSetByCaller(USaudAttributeSet::GetManaAttribute(),
		SaudTags::Data_ManaCost));
}

UGE_SaudDefaultAttributes::UGE_SaudDefaultAttributes()
{
...
UGE_SaudRegeneration::UGE_SaudRegeneration()
{
	DurationPolicy = EGameplayEffectDurationType::Infinite;
	Period = 0.25f;
...
bool USaudCueNotify_Impact::OnExecute_Implementation(AActor* Target,
	const FGameplayCueParameters& Parameters) const
{
	...
	if (!SoundCue.IsNone())
	{
		if (USaudAudioSubsystem* Audio = USaudAudioSubsystem::Get(Target))
		{
