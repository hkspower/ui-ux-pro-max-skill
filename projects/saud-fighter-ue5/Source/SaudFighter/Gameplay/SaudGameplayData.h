...
#include "SaudGameplayData.generated.h"
...
class SAUDFIGHTER_API USaudAttackData : public UPrimaryDataAsset
{
	GENERATED_BODY()

public:
	/** What this strike IS. Its family is the parent tag, so a cracked wall
	    can ask for Saud.Attack.Box and get any punch. */
	...
	{
		return FPrimaryAssetId(TEXT("SaudAttack"), GetFName());
	}
};
 ...
 */
UCLASS(BlueprintType)
class SAUDFIGHTER_API USaudTalentData : public UPrimaryDataAsset
{
	GENERATED_BODY()

public:
	/** Saud.Talent.Vault and so on. This tag IS the record of having it. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Identity")
	...
	TSubclassOf<USaudGameplayAbility> GrantedAbility;
	...
	virtual FPrimaryAssetId GetPrimaryAssetId() const override
	{
		return FPrimaryAssetId(TEXT("SaudTalent"), GetFName());
	}
};
