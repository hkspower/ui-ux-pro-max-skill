...
#include "SaudAttackAbility.generated.h"

class USaudAttackData;
class UGameplayEffect;
class AFighterBase;

/**
 * Every strike in the game, driven by a USaudAttackData.
 *
 ...
 */
UCLASS()
class SAUDFIGHTER_API USaudAttackAbility : public USaudGameplayAbility
{
	GENERATED_BODY()
...
public:
	...
	TObjectPtr<USaudAttackData> Attack;

	/** The next strike of the chain, if the button comes again in recovery. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Attack")
	TSubclassOf<USaudAttackAbility> NextInChain;
