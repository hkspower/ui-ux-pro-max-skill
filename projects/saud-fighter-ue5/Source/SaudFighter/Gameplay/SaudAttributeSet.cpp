...
DEFINE_LOG_CATEGORY_STATIC(LogSaudAttributes, Log, All);

USaudAttributeSet::USaudAttributeSet()
{
	// Saud's level-one numbers, from the browser project's saud.js. Enemies
...
void USaudAttributeSet::ClampToMax(const FGameplayAttribute& Attribute, float& NewValue) const
{
...
void USaudAttributeSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
 ...
 */
void USaudAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	Super::PostGameplayEffectExecute(Data);

	// Data.Target is the component the effect landed on.
	USaudAbilitySystemComponent* ASC = Cast<USaudAbilitySystemComponent>(&Data.Target);
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (Data.Target.HasMatchingGameplayTag(SaudTags::State_Dead)
			|| Data.Target.HasMatchingGameplayTag(SaudTags::State_Invulnerable))
		{
			return;
		}

		float Damage = Raw;
		const bool bGuarding = Data.Target.HasMatchingGameplayTag(SaudTags::State_Blocking);
		const bool bParry = bGuarding
			&& Data.Target.HasMatchingGameplayTag(SaudTags::State_ParryWindow);
		...
		if (bParry)
		{
			...
			if (ASC)
			{
				ASC->SendCombatEvent(SaudTags::Event_Parried, SourceActor, 0.f);
			}
			return;
		}
		...
		if (bGuarding)
		{
			...
			if (ASC)
			{
				ASC->SendCombatEvent(SaudTags::Event_Blocked, SourceActor, Damage);
			}
		}
		...
		if (ASC)
		{
			ASC->SendCombatEvent(SaudTags::Event_HitReceived, SourceActor, Damage);
			if (GetHealth() <= 0.f)
			{
				ASC->SendCombatEvent(SaudTags::Event_Defeated, SourceActor, 0.f);
			}
		}

		UE_LOG(LogSaudAttributes, VeryVerbose, TEXT("%s took %.1f (%s), %.0f/%.0f left"),
