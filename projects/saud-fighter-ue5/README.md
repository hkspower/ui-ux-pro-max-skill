...
The original browser build lives in `../saud-fighter` and is fully playable
...
| Player | `ASaudCharacter` | jab→cross→hook chain, dodge dash, perfect parry, rage meter and finisher, upgrade scaling |
| Enemy AI | `AEnemyFighter` | flanking lanes, attack tokens, hit-and-run, boss phase two |
| Fight styles | `UFightStyleComponent` + `USaudFightStyleData` | range discipline, footwork, range-banded strike selection, reactions |
...
| Profile | `USaudGameInstance` + `USaudSaveGame` | XP, upgrades, abilities, opened gates, ranks, difficulty, stats |
| Scoring | `ASaudGameMode` | S/A/B/C rank, clear bonus, progress writeback |
| Character mesh | `Content/Models/Saud.fbx` + `Content/Textures/Saud/` | rigged, textured hero, Unreal bone names, built by script |
| Remote balance | `USaudConfigSubsystem` | baked tables first, cache second, server third; whole-payload-or-nothing |
| Live link | `USaudLiveLinkSubsystem` | one WebSocket to the server; a changed payload refetches without a restart |
| Save sync | `USaudSaveSyncSubsystem` | the profile uploaded to and fetched from `/v1/saves/{id}` |
...
`Content/Models/Saud.fbx` (and `Saud.gltf` + `Saud.bin`) is a rigged,
textured Saud,
generated by `Tools/blender/build_saud.py`. It needs no Blender install —
`pip install bpy` gives you Blender as a Python module, and the script runs
headless:

```bash
pip install bpy
python3 Tools/blender/build_saud.py            # the hero: 4K colour, 2K normals, 64-sample renders (~15 min)
python3 Tools/blender/build_saud.py --fast     # 1K textures, quick renders, for a look
python3 Tools/blender/build_saud.py --resume   # reload the nine-minute body, redo the rest
python3 Tools/blender/build_saud.py --coarse   # a rough body, to check the later stages in a minute
```

It writes both model files, the textures under `Content/Textures/Saud`, the
...
**Neither model file embeds its textures.** Both reference
`../Textures/Saud/*.png` by relative path, and the glTF export is checked
for images that do not resolve. Embedding them cost 34 MB inside the FBX and
another 34 inside the GLB — the same sixteen files that sit next door, in a
container where git cannot see they are the same, rewritten by every build.
Keep it that way: move the models and the textures together, and import the
PNGs as engine assets rather than letting an importer unpack copies.

**What it is, since 2026-09-10 (Riyadh):** rebuilt from scratch as a hero
asset — the same man as the browser build's `assets/saud.js` (fair skin,
...
bones), six materials and sixteen textures: base colour and roughness for
skin, hair, tee, trousers and trainers at 4096² (hair and trainers at
1024²), each one's normal map at half that, and a 512² eye. The normals are
half size on purpose: the sculpt's shape is low-frequency and survives it,
while the pore and weave bump is finer than a texel at full size and is
stored as per-pixel dither the first mip level throws away — 17.7 MB of it
on the skin alone. Model files and textures together are 17 MB; they were 108
...
- **Bones use Unreal mannequin names** (`pelvis`, `spine_01`, `clavicle_l`,
  ...
  animations onto Saud needs no bone-mapping pass.
...
**The joint table is shared.** `J` at the top of `build_saud.py` is what the
...
**Saud faces −Y.** Toes, face, hair parting and the guard's chin tuck all
...
scale of 1.0 — the FBX is already exported in centimetres. It carries
relative paths to `Content/Textures/Saud` rather than embedded copies, so
import those PNGs as textures in the same pass and wire them into the
material.
...
1. **Generate project files.** Right-click `SaudFighter.uproject` →
   *Generate Visual Studio project files* (or run
   `UnrealBuildTool -projectfiles -project=... -game -engine`).
2. **Build** the `SaudFighterEditor` target, then open the project.
...
5. **Make the Blueprints:**
   - `BP_Saud` from `ASaudCharacter` — mesh, animation, input assets,
...
7. Set **Project Settings → Maps & Modes** to `SaudGameMode` and
   `SaudGameInstance` (already written into `Config/DefaultEngine.ini`).

---

## Data comes from the browser project

Every number the game runs on lives in `../saud-fighter/assets/*.js`, and the
control panel at `../saud-fighter/panel/` edits them. `Content/Data` is
...
| `DT_Fighters.csv` | `enemies.js` + `saud.js` (he is in the roster too) |
...
the browser project — the control panel at `../saud-fighter/panel/` — and
...
**`USaudConfigSubsystem`** pulls the tables. On launch it does three things
...
what it already had and says so in `LogSaudBalance`.

**`USaudLiveLinkSubsystem`** is what makes it live. It holds one socket open
...
**`USaudSaveSyncSubsystem`** carries the profile. `Upload()` PUTs
`FSaudProgress` as JSON under `SchemaVersion` / `Progress` — the two keys
...
> the module list in `SaudFighter.Build.cs` gained `WebSockets` for the
> link.
...
3. An **Apple developer account**, an **App ID** matching `BundleIdentifier` in
   `Config/IOS/IOSEngine.ini` — change `kw.com.sporta.saudfighter` to yours
...
Build/IOS/SaudFighter.ipa`, or drag the `.ipa` into Xcode's *Devices and
Simulators* window.
...
Since 2026-09-19 each one is a **round district**, `SaudArena::DistrictExtent
(Length)` across, built the same way `Tools/fab/lay_out_world.py` builds the
open world's: waves and gates on the spiral `SaudArena::SpiralPoint` draws,
...
at the West door, so `ASaudGameMode::PlaceArrivingPlayer()` only has to find
...
3. Window ▸ Output Log, switch the prompt to *Python*, and:
   ```python
   exec(open(r"/path/to/saud-fighter-ue5/Tools/levels/build_levels.py").read())
   ```
   Or from a Mac terminal with the editor closed:
   ```bash
   "/Users/Shared/Epic Games/UE_5.4/Engine/Binaries/Mac/UnrealEditor-Cmd" \
     SaudFighter.uproject -run=pythonscript -script="Tools/levels/build_levels.py"
   ```
...
not asked for; `ASaudCharacter::SetArenaCircle` is what actually keeps
...
The game opens on a tenth level that is not generated by the script above and
is not one of the nine districts: `L_Prologue`, Saud's life before he fell,
built by `Tools/levels/build_prologue.py` the same way -- run it inside the
editor, or outside it to see the plan. It reads its one row from
`Content/Data/DT_Prologue.json`, hand-authored rather than exported, and its
own `ASaudPrologueGameMode`, set on that level's World Settings rather than
project-wide, sends anyone who has already played it straight to
`L_SouqAlDawar` -- `GameDefaultMap` in `Config/DefaultEngine.ini` now points
...
```python
exec(open(r"/path/to/saud-fighter-ue5/Tools/fab/lay_out_world.py").read())
```
...
all in an outliner folder `SAUD` which it clears first, so it can be re-run
...
- `ASaudCharacter` and `AEnemyFighter` gained `SetArenaFrame(MinX, MaxX,
...
- `ASaudGameMode` listens to every director in the level and scores the
...
browser does. The arriving half is `ASaudGameMode::PlaceArrivingPlayer`,
which reads the `?ArriveAt` option. `FSaudProgress` gained `CurrentStage` and
...
`Gameplay/SaudGameplayTags.h` is the whole vocabulary, declared natively so a
typo is a compile error. The reason tags rather than enums: **an enum can only
answer "which one", a tag answers "is this any kind of X"**. `Saud.Attack.Box`
...
| --- | --- |
| `Saud.Attack.*` | what a strike **is**; its family is its parent tag |
| `Saud.State.*` | what a fighter is doing. Abilities block on these |
| `Saud.Talent.*` | what has been granted — the tag **is** the record |
| `Saud.Event.*` | a moment, dispatched rather than called |
| `Saud.Cue.*` | a moment made visible and audible |
| `Saud.Data.*` | a number carried on an effect (SetByCaller) |

### Attributes

`USaudAttributeSet` holds everything another system may change: health,
...
| Ability | Notes |
| --- | --- |
| `USaudAttackAbility` | every strike, driven by a `USaudAttackData`. Startup → active → recovery are a timeline the system owns. `NextInChain` plus a press during recovery is what makes a jab become a cross become a hook — **there is no combo counter anywhere** |
| `USaudDashAbility` | block-while-moving. DASH LEAP does not add a second dash, it makes this one longer and safer: one button, one move, more of it |
| `USaudGuardAbility` | the parry window opens on the press and shuts 0.2s later whether or not the button is still down. Holding guards at a fifth damage but never parries — that asymmetry is why blocking is a decision |
| `USaudJumpAbility` | **new talent.** Until JUMP is found `JumpPower` is zero and the button does nothing, which is what makes finding it change every area behind you, not just the one ahead. No air jumps: a second one would be a different talent |
| `USaudClimbAbility` | **new talent.** Traces forward at chest height for a wall, then down from above it for a surface. If the drop lands in the climbable band with headroom, the fighter goes up. VAULT opens the ledges a designer placed; CLIMB is the general case, and it comes later because a general answer devalues the specific one if it arrives first |

### Effects, and one damage asset

`UGE_SaudDamage` carries its number by SetByCaller, so **one damage effect
...
The abilities read `USaudAttackData` and `USaudTalentData`, because an asset
...
pick one and rewrite everything, `SaudTalents::TagFor` / `AbilityFor` keep
...
fails to start over a missing sound. Play a stage and `LogSaudAudio` tells
you what is still missing.

`USaudAudioSubsystem` is the player. Three buses (effects, interface, music)
...
(`SaudGameplay::MaxSimultaneousAttackers`) may be swinging at once; the rest
