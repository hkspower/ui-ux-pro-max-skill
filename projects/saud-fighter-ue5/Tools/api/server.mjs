/* SAUD — Kuwait Fighter :: the live API
   ==========================================================================
...
const CONFIG = process.env.SAUD_CONFIG || resolve(HERE, '../../Content/Data/config.json');
const SAVES = process.env.SAUD_SAVES || resolve(HERE, 'saves');
...
/* A save is a profile the game wrote. It is stored as it came, but it has to
   ...
   is the shape USaudSaveGame serialises to. */
...
const pinger = setInterval(() => {
  for(const s of sockets){ try { s.write(frame(0x9, 'saud')); } catch(e){ sockets.delete(s); } }
}, 25000);
pinger.unref();
...
server.listen(PORT, HOST, () => {
  ...
  try { rev = payload().revision; } catch(e){}
  watchPayload();
  console.log(`SAUD live API on http://${HOST}:${PORT}   ws://${HOST}:${PORT}/v1/live`);
