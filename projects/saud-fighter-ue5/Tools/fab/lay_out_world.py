"""SAUD — Kuwait Fighter :: lay the world over a map from Fab
==============================================================================
...
day (Source/SaudFighter/Combat/SaudArena.h) and the map had to stop with
...
HOW TO USE IT
  ...
  4. Window ▸ Output Log, switch the prompt to Python:
     ...
     all sit in the outliner folder SAUD) are removed first, so it can be run
...
# (Source/SaudFighter/Combat/SaudArena.h), so the map had to stop too — a
...
def spiral(t, extent, phase):
    ...
    The same curve as SaudArena::SpiralPoint in the game and District.Spiral
...
def draw(stages, P, discs, path):
    ...
    d.text((16, 16), "SAUD on a Fab map: nine round districts, souq at the top, clockwise east; "
