...
BROWSER = os.path.abspath(os.path.join(PROJECT, "..", "saud-fighter"))
...
# Duplicated from Combat/SaudArena.h and Tools/levels/*.py on purpose: see
...
class Souq:
    ...
    def scene(self, fighters_dir, blend_out, render_name="souq-fight.png"):
        """The fight at wave 2: Saud and the wave's men, posed through their
        control rigs, in the built souq."""
        bpy = self.bpy
        from mathutils import Vector
        import build_saud as legacy, rig_full_ik as CR
        P = self.P
        ...
        # Saud, in front of him for even j and behind for odd, in browser
        # pixels taken to centimetres by PX. Saud is at the wave. Nothing
        # here is chosen.
        ...
        for i, (kind, (fwd, side)) in enumerate(men):
            ...
            # every man squares up to Saud; Saud squares up to the first of them
            f0 = men[1][1][0]
