/* SAUD — Kuwait Fighter :: assets -> Unreal
   ==========================================================================
...
const WEB = resolve(UE5, '../saud-fighter');
...
function fighters(A){
  ...
  // Saud himself, so the table is the whole roster rather than the enemies
  // only. His moves are the punch chain the player actually throws.
  const P = A.saud;
  rows.push(['Saud', P.name, P.ar, P.base.hp, P.base.pow, cm(P.base.spd),
...
/* Points every archetype at the style asset build_data_assets.py generates
   ...
   and Saud gets none, because the player is not driven by one. */
...
function talents(A){
  ...
  const rows = Object.entries(A.talents.abilities).map(([k, t]) => {
    if(!ABILITY[k]) die(`talent '${k}' has no EAbility in SaudTypes.h — add it there first`);
...
function gates(A){
  ...
  const rows = Object.entries(A.talents.gates).map(([k, g]) => {
    if(!GATE[k]) die(`gate '${k}' has no EGateType in SaudTypes.h`);
