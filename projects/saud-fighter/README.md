...
A 2D mobile Metroidvania beat-'em-up. Saud is a young Kuwaiti fighter working his
...
spinning finisher that hits everyone around Saud.

**Perfect parry** is the skill ceiling. Press block within a fifth of a second of
an incoming blow and Saud takes no damage at all, the attacker is staggered out
...
title (ROOKIE → CHAMPION) shown under Saud's name.
...
It sets Saud's punches alight: ×1.55 damage, a little more reach and
...
catch fire the moment Saud found it.
...
armed Saud hits harder and reaches further but gives up the kick game, so
...
would have had every enemy hitting like a crowbar the moment Saud picked one
up.
...
opt-in rather than riding along with `pants`; Saud, the Kickboxer and AL-SAQR
have it, everyone else is in jeans.

Nobody wears boxing gloves with jeans, so `hands` picks `bare` knuckles or
taped `wraps` instead. Saud keeps red, now as tape.

The kit is a `look` object on each `TYPES` entry, applied in `makeEnemy`, and
it uses the same flags Saud does — plus `hoodie`, `jacket`, `hands`, `stripe`,
...
under `saud_kuwait_fighter_v1` — the game runs fine without storage (private
...
| File | Holds |
| --- | --- |
| `assets/saud.js` | the player: base stats, what each upgrade level adds, palette and kit |
...
press/saud-hero-kick.png          Saud mid-roundhouse on a dark plate
press/saud-guard.png              Saud in his fighting stance
...
lighting rigs (`THEME`), the fighters' kits (`saud.js`, `enemies.js`) and the
...
| **Enemies** | Threat: how much health an archetype takes off you before it goes down, one on one. That is damage output × survival time, and it is not the same order as HP. Also flags anything that would kill a level-1 Saud alone. |
