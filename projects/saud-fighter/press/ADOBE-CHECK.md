# Adobe check — Saud's design

Run 2026-09-10, 02:10 Riyadh, against the key art in this folder and the
character renders in `../saud-fighter-ue5/Docs/renders/`. What Adobe's tools
...
| `chamfer-gothic-ludlow` (the SAUD title) | **not found** | Impact |
...
| --- | --- | --- | --- |
| SAUD | `#e0b34a` on `#141c2d` | **8.7:1** | passes AAA |
...
the poster relies on the warm radial light behind Saud to fill it.
...
## Character (`saud-guard-3d.png`, `saud-kick-3d.png`, `saud-apose-3d.png`)

Measured against `../saud-fighter-ue5/CLAUDE.md`, which is the brief for
this body: eight heads, real anatomy, hands that are hands, PBR skin and
cloth, no rounded cartoon shapes.

- **Proportion is right.** From the joint table in `build_saud.py`, crown
