#!/usr/bin/env python3
"""SAUD -- the preview booklet, built rather than laid out.

WHAT THIS MAKES: `saud-preview.pdf`, eight pages, the thing you hand
...
own sentences -- which is not data and does not pretend to be.
    ...
    python3 build_preview.py                 # -> saud-preview.pdf
    python3 build_preview.py --png out/      # every page as a PNG, to look at
    python3 build_preview.py --html only.html

A NOTE ON THE ART DIRECTION. `../../../saud-fighter-ue5/CLAUDE.md` says the
...
UE5 = os.path.normpath(os.path.join(BROWSER, "..", "saud-fighter-ue5"))
...
<div class="abs arh gold" style="left:120px;top:190px;font-size:210px;
     line-height:1.32;letter-spacing:0;text-align:left;width:780px">سعود</div>
<div class="abs dsp" style="left:132px;top:486px;font-size:186px;line-height:.82;
     color:{C['white']}">SAUD</div>
...
{F.saud_falling(cx0 + cw / 2 + 16, cy0 + 344, 0.70, rot=66)}
...
{foot(C, 'SAUD — KUWAIT FIGHTER', 'SPORTA · ALMUHALLAB CODE COMPANY · KUWAIT', 2)}
</section>"""
...
<div class="card" style="left:112px;top:794px;width:400px;height:228px;padding:26px">
  ...
  <div class="ar" style="margin-top:10px;font-size:22px;font-weight:600;
    ...
    كل طريق مغلق حتى يتعلّم سعود ما يفتحه، وما يفتحه يقع دائمًا خلف قتال.</div>
  <div class="lat" style="margin-top:10px;font-size:17px;line-height:1.42;
       color:rgba(244,241,232,.62)">
    Every link is sealed until Saud learns the thing that opens it, and the
...
BOSS_LINES = {
    "Saqr": ("كل شيء فيه سيقان، ولا صبر عنده. سقط هو أيضًا قبل سنوات، وكفّ عن "
             "السؤال — وهذا مصدر نفاد صبره. هو ما سيصيره سعود بعد سنة، ويقول "
             "له ذلك، ولا يصل.",
             ...
             "Saud becomes in a year, and he says so, and it does not land."),
...
<div class="abs lat" style="left:{x + 24}px;top:848px;width:{pw - 48}px;
    ...
    rest = [f for f in fighters if f["bIsBoss"] == "false" and f["Name"] != "Saud"]
    rw = (1696 - 8 * 14) / 9
    roster = "".join(f"""
...
{''.join(plates)}
{roster}
{foot(C, 'SAUD — KUWAIT FIGHTER', 'HIT POINTS AND MOVE LISTS FROM DT_FIGHTERS', 6)}
</section>"""
...
<div class="abs arh gold" style="left:0;right:0;top:160px;text-align:center;
     font-size:176px;line-height:1.34;direction:rtl">سعود</div>
<div class="abs dsp" style="left:0;right:0;top:428px;text-align:center;
     font-size:96px;color:{C['white']};line-height:1">SAUD</div>
...
<title>SAUD — Kuwait Fighter · معاينة</title>
...
def render(html, out_pdf, pngs=None):
    ...
    tmp = tempfile.mkdtemp(prefix="saud-preview-")
...
def main():
    ...
    ap.add_argument("--pdf", default=os.path.join(HERE, "..", "saud-preview.pdf"))
