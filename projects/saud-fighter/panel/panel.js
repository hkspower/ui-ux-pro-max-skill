/* SAUD — Kuwait Fighter :: control panel
   ==========================================================================
...
/* ------------------------------------------------------------- asset table */
var DOMAINS = [
  { k:'saud', n:'Saud', ico:'✦', g:'ASSET_SAUD', file:'assets/saud.js',
    shape:'single',
...
/* Saud's sustained damage over a full chain, at a given upgrade level. A
...
function saudHP(level, vit){
  return D.saud.base.hp + D.saud.perLevel.vit * (vit||0) +
...
function insight(){
  ...
  var fns = { hits:iHits, enemies:iEnemies, levels:iLevels, upgrades:iUpgrades,
              world:iWorld, stages:iStages, weapons:iWeapons, saud:iSaud,
              talents:iTalents };
...
function iEnemies(){
  ...
  var hp1 = saudHP(1, 0), hpMax = saudHP(D.levels.max, D.upgrades.maxLevel);
  ...
  out += '</table>' +
    m('Saud HP, level 1', n0(hp1)) +
    m('Saud HP, fully built', n0(hpMax)) +
  ...
  out += deadly.length
    ? '<div class="verdict warn"><b>ONE-ON-ONE LETHAL AT LEVEL 1</b>' +
      ...
      'Saud down alone if he never blocked or dodged. Fine for a boss, worth ' +
      'a look on anything that spawns in a first wave.</div>'
    : '<div class="verdict ok"><b>SURVIVABLE</b>A level-1 Saud can out-trade ' +
